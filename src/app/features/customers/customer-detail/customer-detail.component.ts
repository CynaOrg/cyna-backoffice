    ...
    <div>
      @if (loading()) {
        ...
        <div class="mt-6 bg-surface rounded-xl border border-border-light shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold">
              {{ 'CUSTOMERS.SUBSCRIPTION_HISTORY' | translate }}
            </h3>
            <a
              [routerLink]="['/subscriptions']"
              class="text-sm text-primary hover:text-primary-hover"
            >
              {{ 'CUSTOMERS.VIEW_ALL_SUBSCRIPTIONS' | translate }}
            </a>
          </div>
          ...
          @if (subscriptionsLoading()) {
            ...
            <p class="py-8 text-center text-text-muted text-sm">
              {{ 'CUSTOMERS.NO_SUBSCRIPTIONS' | translate }}
            </p>
          } @else {
            <div class="overflow-x-auto">
              <table class="w-full">
...
export class CustomerDetailComponent implements OnInit {
  ...
  subscriptionsLoading = signal<boolean>(false);
  ...
  loadSubscriptions(userId: string): void {
    ...
    this.api
      .get<Subscription[]>(`admin/users/${userId}/subscriptions`)
      .pipe(finalize(() => this.subscriptionsLoading.set(false)))
      .subscribe({
        next: (res) => {
          this.subscriptions.set(Array.isArray(res) ? res : []);
        },
        error: () => {
